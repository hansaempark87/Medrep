...
- 등급 기준 상향 조정 (S: 95점 이상, A: 80-94점, B: 65-79점, C: 50-64점, D: 50점 미만)
- 스코어링 가중치 조정 (H-index: 20→30점, 인용수: 25→30점)
- 질환별 전문성 가산점 시스템 도입 (최대 +10점)
  - 태그에 질환명 포함: +5점
  - 태그에 질환 카테고리 포함: +3점
  - 학회에 질환 관련 키워드: +2점
- 등급 설명 업데이트 (새로운 기준 반영)
- 예시: 김효수 교수 A등급(90점), 최동훈 교수 S등급(95점, 전문성 가산점 +5)

### v13.1 (2026-02-14)
- **사용성 및 정확도 대폭 개선**
- 이상지질혈증 설명 강화 (상세하고 친절한 설명으로 변경)
- 점수 표시 제거, 등급만 표시 (점수 불일치 문제 해결)
- 연구 데이터 설명을 일반인이 이해하기 쉽게 개선
- 헤더에 뒤로가기 버튼 추가 (상세 페이지에서 표시)
- 연구 지표 툴팁 추가 (논문, 인용, H-index 용어 설명)
- 모든 숫자에 단위 표시 (편, 회, 건)
- 점수 링을 프로필 아바타로 변경
- 정렬 라벨 변경 (점수순 → 등급순)
